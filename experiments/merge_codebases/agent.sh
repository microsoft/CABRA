...
if [[ ${#copilot_models[@]} -eq 0 ]]; then
	copilot_models=("claude-opus-4.8")
fi
...
session_prefix="agent_${experiment_name:-merge_code_sweep}"
...
for model_name in "${copilot_models[@]}"; do
	...
	cmd+=(--agent-kwargs timeout=10800)
	...
	if [[ "${tmux_debug}" == "true" ]]; then
		tmux_cmd="tmux set-option -w -t '${session}' remain-on-exit on; ${tmux_cmd}"
	fi
